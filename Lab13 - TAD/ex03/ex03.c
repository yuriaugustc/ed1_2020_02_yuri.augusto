...
int main(){
    ...
    Sleep(5000);
    TMat2D *sum = mat2d_sum_2_mat(mat, mat1);
    if(sum == NULL)
    ...
    double columns = mat2d_sum_columns(mat1);
    if(columns != -1)
        printf("\nSeeing the result: %.2lf\n", columns);
    else
    ...
    int gcolumn = mat2d_get_columns(mat2);
    if(gcolumn != -1)
        printf("\nSeeing the result: %d\n", gcolumn);
    else
